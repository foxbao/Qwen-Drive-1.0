...
OUTPUT_DIR="${OUTPUT_DIR:-$ROOT/outputs/planner-real-smoke}"
LIMIT="${LIMIT:-1}"
STEPS="${STEPS:-5}"
...
echo "Running real planner smoke test in conda environment: $CONDA_ENV"
echo "Model: $MODEL_DIR"
echo "Initial planner: $PLANNER_DIR"
echo "Output: $OUTPUT_DIR"

exec conda run --no-capture-output -n "$CONDA_ENV" \
  python "$ROOT/scripts/train_planner_real_smoke.py" \
  --model "$MODEL_DIR" \
  --planner "$PLANNER_DIR" \
  --scenes "$SCENES" \
